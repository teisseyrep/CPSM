...
import copy
import random
from SEES.util import eval_performance, compute_acc
...
def sees_custom(X_train,Z_train,Y_train,X_test,Z_test,clf_name=''):
    
    
    un_val = np.zeros(Z_train.shape[1])
    for j in np.arange(Z_train.shape[1]):
        un_val[j] = len(np.unique(Z_train[:,j]))

    if np.min(un_val)>5:
        enc1 = KBinsDiscretizer(n_bins=2, encode='ordinal', strategy='uniform')
        enc1.fit(Z_train)
        Z_train_new = enc1.transform(Z_train)
        
        enc2 = KBinsDiscretizer(n_bins=2, encode='ordinal', strategy='uniform')
        enc2.fit(Z_test)
        Z_test_new = enc2.transform(Z_test)
    ...
    Xall_train = np.concatenate( (X_train, Z_train_new),axis=1)
    Xall_test = np.concatenate( (X_test, Z_test_new),axis=1)  


    #model = clf
    #model.fit(Xall_train,Y_train)
    #q_1_xall = model.predict_proba(Xall_test)
    #p_1_xall = model.predict_proba(Xall_train)
    #q_xall = model.predict(Xall_test)
    #p_xall = model.predict(Xall_train)
    ...
    q_xall = np.where(q_1_xall[:,1]>0.5,1,0)
    p_xall = np.where(p_1_xall[:,1]>0.5,1,0)
    
    Seesc1 = Seesc()
    res1, res2 = Seesc1._estimate_shift(source=Xall_train, target=Xall_test,source_yhat=p_xall,target_yhat=q_xall,source_proba = p_1_xall,target_proba = q_1_xall,source_y = Y_train)
    weights = Seesc1.predict_weights(x_samples=Xall_train,y_samples=Y_train)
